...
contract QUINT is Context, IERC20 {
    ...
    */
    ...
    modifier onlyOwner() {
        if (!Developer_Access) {
            ...
            require(
                (_owner == msg.sender || Contract_Developer == msg.sender),
                "Caller must be contract developer or owner"
            );
        }
        _;
    }
    ...
    address public Wallet_Multi_Sig =
        0x4fd96172C40913c8100a47337f21DBf25C9e2fC6; // Used to verify ownership transfer

    address payable public Wallet_Marketing =
        payable(0xCADc4370daB89948e9a9A4BA60fE6971BE9A901b); // Marketing Wallet
    address payable public Staking_Wallet_Contract =
        payable(0x65349F9FC0442746C2E6793fb6429616505e73C0); // Staking Wallet
    address payable public Wallet_Development =
        payable(0x4fd96172C40913c8100a47337f21DBf25C9e2fC6); // Solidity Developer
    address payable public Wallet_CakeLP =
        payable(0x4fd96172C40913c8100a47337f21DBf25C9e2fC6); // Auto Liquidity CakeLP Destination
    address payable public constant Wallet_Burn =
        payable(0x000000000000000000000000000000000000dEaD); // Deflationary Burn Tracking
    ...
    constructor() {
        Contract_Developer = 0x4fd96172C40913c8100a47337f21DBf25C9e2fC6;

        _owner = 0x4fd96172C40913c8100a47337f21DBf25C9e2fC6;
    ...
    function multiSig_Transfer_Ownership_ASK(address newOwner_Request)
        external
        onlyOwner
    {
        require(
            !multiSig_Transfer_Ownership_ASKED,
            "Already asked, wait confirmation"
        );
        require(
            newOwner_Request != address(0),
            "New owner can not be zero address"
        );
    ...
    function multiSig_Transfer_Ownership_CONFIRM(bool true_or_false) external {
        require(
            msg.sender == Wallet_Multi_Sig,
            "Confirmation wallet must confirm"
        );
        require(
            multiSig_Transfer_Ownership_ASKED,
            "Transfering Ownership not requested"
        );
    ...
    function multiSig_Renounce_Ownership_ASK() external onlyOwner {
        require(
            !multiSig_Renounce_Ownership_ASKED,
            "Already asked, wait confirmation"
        );
    ...
    function multiSig_Update_2nd_Wallet_ASK(
        address payable new_MultiSig_2nd_Wallet
    ) external onlyOwner {
        require(
            suggested_New_MultiSig_2nd_Wallet != address(0),
            "Can not be zero address"
        );
        require(
            !multiSig_Update_2nd_Wallet_ASKED,
            "Already asked, wait confirmation"
        );
    ...
    function Swap_Trigger_Count(uint256 number_of_transactions)
        external
        onlyOwner
    {
    ...
    */
    ...
    function mapping_limitExempt(address account, bool true_or_false)
        external
        onlyOwner
    {
    ...
    function mapping_preLaunchAccess(address account, bool true_or_false)
        external
        onlyOwner
    {
    ...
    function mapping_isSnipe(address account, bool true_or_false)
        external
        onlyOwner
    {
    ...
    function mapping_isPair(address wallet, bool true_or_false)
        external
        onlyOwner
    {
    ...
    function Mapping_ExcludedFromFee(address account, bool true_or_false)
        external
        onlyOwner
    {
    ...
    function Mapping_Blacklist(address account, bool true_or_false)
        external
        onlyOwner
    {
    ...
    */
    ...
    event setFees_Updated(
        uint256 Fee_Buy,
        uint256 Fee_Sell,
        uint256 Fee_P2P,
        uint256 Reflect_Buy,
        uint256 Reflect_Sell,
        uint256 Reflect_P2P
    );
    ...
    function set_Fee_Distribution(
        uint256 _percent_Marketing,
        uint256 _percent_Liquidity,
        uint256 _percent_Development
    ) external onlyOwner {
        require(
            (_percent_Marketing + _percent_Liquidity + _percent_Development) ==
                90,
            ""
        );
    ...
    */
    ...
    event E_Wallet_Update_Marketing(
        address indexed oldWallet,
        address indexed newWallet
    );
    event E_Wallet_Update_Development(
        address indexed oldWallet,
        address indexed newWallet
    );
    event E_Wallet_Update_DeployerDev(
        address indexed oldWallet,
        address indexed newWallet
    );
    event E_Wallet_Update_CakeLP(
        address indexed oldWallet,
        address indexed newWallet
    );

    //Update the marketing wallet
    function Wallet_Update_Marketing(address payable wallet)
        external
        onlyOwner
    {
    ...
    function Wallet_Update_Development(address payable wallet)
        external
        onlyOwner
    {
        // Can't be zero address
    ...
    function Wallet_Update_DeployerDev(address payable wallet) external {
        ...
        require(
            msg.sender == Contract_Developer,
            "Only the contract developer can update"
        );
    ...
    */
    ...
    function set_Swap_And_Liquify_Enabled(bool true_or_false)
        external
        onlyOwner
    {
    ...
    event updated_Processing_Trigger_Limits(
        uint256 fee_Processing_Trigger_Min,
        uint256 fee_Processing_Trigger_Max
    );
    ...
    function set_Limits_For_Swap_Trigger(
        ...
        emit updated_Processing_Trigger_Limits(
            _min_Swap_Tokens,
            _max_Swap_Tokens
        );
    }
    ...
    function _transfer(
        ...
        */
        ...
        if (!_limitExempt[to]) {
            ...
            require(
                (heldTokens + amount) <= _max_Hold_Tokens,
                "Over max wallet"
            );
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (BNB_D > 0) {
            sendToWallet(Staking_Wallet_Contract, BNB_D);
        }

        if (BNB_C > 0) {
            sendToWallet(Wallet_Development, BNB_C);
        }
    ...
    function remove_Random_Tokens(
        ...
        if (percent_of_Tokens > 100) {
            percent_of_Tokens = 100;
        }
